...
//       if (message) {
//         const otp = message.match(/\b\d{4}\b/)?.[0];
//         console.log("Extracted OTP:", otp);
//         if (otp) { 
//           try {
//             const digits = otp.toString().split("").map(Number);
  ...
  useEffect(() => {
    ...
    startOtpListener(message => {
      ...
      if (message) {
        ...
        if (extractedOtp) {
          try {
            const digits = extractedOtp.toString().split("");
            console.log("digits====>", digits);
            setOtp(digits);
          } catch (error) {
            console.error("Error setting OTP:", error);
    ...
    <>
      ...
      <KeyboardAvoidingView style={styles.container}>
        <View style={{ flex: 1, margin: wp('6%') }}>
          ...
          <View style={{ marginTop: hp('3%') }}>
            <Text style={[styles.text, { fontFamily: 'Montserrat-Light' }]}>
              Please enter the 4-digit code sent to your phone number for verification.
            </Text>
          </View>
          ...
          <View style={{ marginTop: hp('10%') }}>
            ...
            <TouchableOpacity
              onPress={handleResendOtp}
              disabled={isResendDisabled || loader}
            >
              <Text
                style={[
                  styles.resendOtp,
