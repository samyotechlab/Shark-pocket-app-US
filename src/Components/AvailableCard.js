...
export default function AvailableCard({gameData,status,index}) {
    ...
    return (
        <View>
            <LinearGradient
                colors={status == 2 || status == 4? colors : ['#F38424', '#F7A552', '#F9D479']}
                start={{ x: 0, y: 0.5 }}
                end={{ x: 0.8, y: 1 }}
                style={[styles.card,{borderColor:status == 2 || status == 4?border:'#F2E30B'}]}
            >
                <View style={styles.content}>
                    ...
                    <View style={styles.textContainer}>
                        <Text style={styles.headerText}>
                            GET {status == 4 ? gameData.game_winning_cost :gameData.enroll_cost} & PLAY NOW
                        </Text>
                        <Text style={styles.description}>
                            You will get the {status == 4 ? gameData.game_winning_cost :gameData.enroll_cost} prize money
                        ...
                        <View style={styles.buttonContainer}>
                            <View style={styles.button}>
                                <Text style={styles.buttonText}>{status == 4 ? gameData.game_winning_cost :gameData.enroll_cost} CASH WIN</Text>
                            </View>
                        </View>
                    </View>
                </View>
